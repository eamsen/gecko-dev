...
class ContentDelegateTest : BaseSessionTest() {
    ...
    @Test fun autofill() {
        ...
        mainSession.evaluateJS("this.promises = []");

        // Set up promises to monitor the values changing.
        autoFills.forEach { entry ->
            // Repeat each test with both the top document and the iframe document.
            mainSession.evaluateJS("""
                [document, document.querySelector('#iframe').contentDocument].map(doc =>
        ...
        val result = mainSession.evaluateJS("Promise.all(this.promises)") as JSONArray
        // Wait on the promises and check for correct values.
        for (i in 0 until result.length()) {
            val entry = result.get(i) as JSONArray
            val key = entry.get(0)
            val actual = entry.get(1)
            val expected = entry.get(2)
            val eventInterface = entry.get(3) as String
