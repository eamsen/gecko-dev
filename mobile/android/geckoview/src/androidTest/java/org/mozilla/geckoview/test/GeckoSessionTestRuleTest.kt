...
class GeckoSessionTestRuleTest : BaseSessionTest(noErrorCollector = true) {
    ...
    @Test fun evaluateJS_supportPromises() {
        ...
        assertThat("Promise should be resolved",
            sessionRule.session.evaluateJS("new Promise(resolve => resolve(0))") as Double,
            equalTo(0.0));
        ...
        val result = sessionRule.session.evaluateJS("Promise.all(this.promises)") as JSONArray
        assertThat("Promise result should be correct", result.getInt(0), equalTo(0))
        assertThat("Promise result should be correct", result.getInt(1), equalTo(1))
    }
    ...
    fun evaluateJS_throwOnRejectedPromise() {
        sessionRule.session.loadTestPath(HELLO_HTML_PATH)
        sessionRule.session.waitForPageStop()
        sessionRule.session.evaluateJS("Promise.reject('foo')")
    }
