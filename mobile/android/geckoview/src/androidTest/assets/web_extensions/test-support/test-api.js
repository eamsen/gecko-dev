...
function linkColorFrameScript() {
  addMessageListener("HistoryDelegateTest:GetLinkColor", function onMessage(message) {
      ...
      const color = content.windowUtils.getVisitedDependentComputedStyle(element, "", "color");
    ...
    return {
      test: {
        ...
        async getLinkColor(uri, selector) {
          const frameScript = `data:text/javascript,(${encodeURI(linkColorFrameScript)}).call(this)`;
          Services.mm.loadFrameScript(frameScript, true);

          return new Promise((resolve, reject) => {
            const onMessage = message => {
              Services.mm.removeMessageListener("HistoryDelegateTest:GetLinkColor",
                                                onMessage);
              if (message.data.ok) {
            ...
            Services.mm.addMessageListener("HistoryDelegateTest:GetLinkColor", onMessage);
            Services.mm.broadcastAsyncMessage('HistoryDelegateTest:GetLinkColor',
                                              { uri, selector });
          });
        },
