...
public:
                      ...
                      const TimeUnit& aDuration) override
  {
    ...
    RefPtr<layers::Image> img = mImageContainer->CreateImage(
    ...
    layers::SurfaceTextureImage* stImg =
        static_cast<layers::SurfaceTextureImage*>(img.get());
    stImg->SetData(data);
    ...
    if (WantCopy()) {
      ...
      layers::EGLImageImage* typedImg =
        static_cast<layers::EGLImageImage*>(img.get());
